...
import { PACKAGE_TYPE_LIBRARY, PACKAGE_TYPE_TEMPLATE } from '../services/types.js';
...
export class SearchCommand {
    ...
    async displayDetailedResults(packages) {
        ...
        for (let i = 0; i < packages.length; i++) {
            ...
            if ('homepage' in pkg) {
                console.log(`   Homepage: ${chalk.blue.underline(pkg.homepage)}`);
            }
            if ('repository' in pkg) {
                console.log(`   Repository: ${chalk.blue.underline(pkg.repository)}`);
            }
            if ('license' in pkg) {
            ...
            if (pkg.tags && pkg.tags.length > 0) {
                console.log(`   Tags: ${chalk.magenta(pkg.tags.map((t) => `#${t}`).join(' '))}`);
            }
